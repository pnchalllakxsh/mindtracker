...
import { Bell, Calendar, Plus, LogOut, Settings } from 'lucide-react';
...
/**
 * Sticky top header with date display, notification bell,
 * quick-log button, and user account dropdown.
 */
    ...
    <header className="flex items-center justify-between px-8 py-4 bg-white/50 dark:bg-slate-900/50 backdrop-blur-sm sticky top-0 z-10 border-b border-slate-100 dark:border-slate-800">
      ...
      <div className="flex items-center gap-4">
        <div className="hidden md:flex items-center gap-2 px-3 py-1.5 bg-white dark:bg-slate-800 rounded-full shadow-sm text-sm text-slate-600 dark:text-slate-300 font-medium border border-slate-100 dark:border-slate-700">
          <Calendar className="w-4 h-4 text-violet-600" aria-hidden="true" />
          {dateStr}
        </div>

        <button
          className="p-2 bg-white dark:bg-slate-800 rounded-full shadow-sm text-slate-500 hover:text-violet-600 border border-slate-100 dark:border-slate-700 transition-colors"
        ...
        <DropdownMenu>
          <DropdownMenuTrigger className="rounded-full focus:ring-2 focus:ring-violet-500 outline-none" aria-label="Account menu">
          ...
          <DropdownMenuContent align="end" className="w-56">
            <div className="px-2 py-1.5 text-sm font-semibold">{user?.name ?? 'My Account'}</div>
            <DropdownMenuSeparator />
            <Link href="/settings">
